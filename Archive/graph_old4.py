...
The data is stored in 2 dicts: ._map_data and ._id_index:

._map_data:
In ._map_data, the keys are tuples containing the vertex coordinates: (latitude, longitude). 

The value (k) of each key is a list consisting of 2 lists and 1 int (optional):
    => The list at k[0] contains the coordinates (tuples) of all the neighbours of that vertex.
       List is being used as neighbours could need updating as the map progresses.
    => The list at k[1] contains the names of the edges (streets) through which
       the neighbours stored in k[0] are connected. The indexing of k[0] and k[2]
       is summetrerical. 
    => The int at k[2] is the ID of that vertex as mentioned in the database. This is added
       ...
       e.g. The vertex v is connected to ._map_data[v][0][i] through ._map_data[v][1][i]
       street name. If v had a ID/handle given, it is stored at ._map_data[v][2] 
